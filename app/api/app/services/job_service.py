...
class JobService:
    ...
    def create_job(self, source_type: str, source_reference: str, auto_repair: bool) -> JobCreateResponse:
        ...
        with self._lock:
            ...
            self._simulate_analysis(record)
            if auto_repair:
                self._simulate_repair(record)

        return JobCreateResponse(job_id=job_id, status=record.status, created_at=created_at)
    ...
    def trigger_repair(self, job_id: str, repair_strategy: str) -> JobStatusResponse:
        with self._lock:
            ...
            self._simulate_repair(record, repair_strategy=repair_strategy)
    ...
    def _simulate_analysis(self, record: JobRecord) -> None:
    ...
    def _simulate_repair(self, record: JobRecord, repair_strategy: str = "balanced") -> None:
