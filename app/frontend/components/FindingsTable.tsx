...
type SortCol = "severity" | "file" | "tool" | "rule";
...
export function FindingsTable({ findings, selectedFinding, onSelect }: Props) {
  ...
  // Unique basenames for file filter dropdown
  ...
  const filtered = useMemo(() => {
    ...
    if (search.trim()) {
      ...
      result = result.filter(
        (f) =>
          basename(f.file).toLowerCase().includes(q) ||
    ...
    return result;
  }, [findings, severityFilter, fileFilter, search]);
            ...
            {pageItems.map((f, i) => {
              ...
              return (
                ...
                >
                  ...
                  <td
                    className="px-3 py-2 font-mono text-gray-600 dark:text-gray-400 truncate"
                    title={`${f.file}:${f.line}`}
                  >
                    {basename(f.file)}:{f.line}
                  </td>
                  <td className="px-3 py-2 font-mono text-gray-500 dark:text-gray-400 truncate">
                    {f.rule_id}
                  </td>
                  <td className="px-3 py-2 text-gray-700 dark:text-gray-300 truncate">
                    {f.message}
                  </td>
                </tr>
              );
